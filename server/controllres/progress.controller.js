...
const addLectureProgress = asyncHandler(async (req, res) => {
  ...
  const alreadyCompleted = progressStored?.lecturesCompleted?.some(
    (lecture) => lecture.toString() === lectureId.toString(),
  );

  if (
    !alreadyCompleted &&
    course?.lectures?.length === progressData?.lecturesCompleted?.length
  ) {
  ...
  return res
    ...
    .json(
      new ApiResponse(
        200,
        courseProgress,
        "Course Progress has been fetched successfully",
      ),
    );
});
